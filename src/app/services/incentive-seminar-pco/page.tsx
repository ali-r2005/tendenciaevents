...
export const metadata: Metadata = {
  title: dictionary.incentiveSeminarPcoPage.meta.title,
  description: dictionary.incentiveSeminarPcoPage.meta.description,
};

export default function IncentiveSeminarPco() {
    ...
    <main className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 py-6 space-y-24 sm:space-y-32">
          ...
          <div className="md:col-span-5">
            <div className="w-full h-64 sm:h-80 bg-brand-canvas rounded-custom overflow-hidden border border-brand-border">
              <img
                ...
                alt="Incentive Seminar and PCO services in Morocco"
      ...
      <section className="space-y-8">
        <h2 className="font-display text-3xl sm:text-5xl font-normal text-brand-neutral tracking-tight">
          {t.incentiveSeminarPcoPage.includesLabel}
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {t.incentiveSeminarPcoPage.includes.map((item) => (
            <div
              key={item}
      ...
      <section className="space-y-8">
        <h2 className="font-display text-3xl sm:text-5xl font-normal text-brand-neutral tracking-tight">
          {t.incentiveSeminarPcoPage.relatedLabel}
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {t.incentiveSeminarPcoPage.related.map((service) => (
            <a
              key={service.title}
              href={service.href}

...
export const metadata: Metadata = {
  title: dictionary.destinationManagementPage.meta.title,
  description: dictionary.destinationManagementPage.meta.description,
};
    ...
    <main className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 py-6 space-y-24 sm:space-y-32">
      ...
      <section className="space-y-8 pt-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-center">
          <div className="md:col-span-7 space-y-2">
            ...
            >
              {t.destinationManagementPage.cta}
            </a>
          </div>
          <div className="md:col-span-5">
            <div className="w-full h-64 sm:h-80 bg-brand-canvas rounded-custom overflow-hidden border border-brand-border">
              <img
                ...
                alt="Destination Management in Morocco"
      ...
      <section className="space-y-8">
        <h2 className="font-display text-3xl sm:text-5xl font-normal text-brand-neutral tracking-tight">
          {t.destinationManagementPage.includesLabel}
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {t.destinationManagementPage.includes.map((item) => (
            <div
              key={item}
      ...
      <section className="space-y-8">
        <h2 className="font-display text-3xl sm:text-5xl font-normal text-brand-neutral tracking-tight">
          {t.destinationManagementPage.relatedLabel}
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {t.destinationManagementPage.related.map((service) => (
            <a
              key={service.title}
              href={service.href}

...
export const metadata: Metadata = {
  title: dictionary.eventProductionPage.meta.title,
  description: dictionary.eventProductionPage.meta.description,
};
    ...
    <main className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 py-6 space-y-24 sm:space-y-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-center">
          <div className="md:col-span-7 space-y-2">
            ...
            <div className="flex flex-wrap gap-4 mt-4">
              ...
              >
                {t.eventProductionPage.cta}
              </a>
              <a
                href="/contact-us"
                className="inline-block border border-brand-border hover:border-brand-green transition-colors px-8 py-3.5 rounded-custom text-brand-neutral text-[13px] font-medium tracking-wide uppercase"
              >
                {t.eventProductionPage.cta2}
              </a>
            </div>
          </div>
          <div className="md:col-span-5">
            <div className="w-full h-64 sm:h-80 bg-brand-canvas rounded-custom overflow-hidden border border-brand-border">
              <img
                ...
                alt="Event Production services in Morocco"
      ...
      <section className="space-y-8">
        <h2 className="font-display text-3xl sm:text-5xl font-normal text-brand-neutral tracking-tight">
          {t.eventProductionPage.includesLabel}
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {t.eventProductionPage.includes.map((item) => (
            <div
              key={item}
      ...
      <section className="space-y-8">
        <h2 className="font-display text-3xl sm:text-5xl font-normal text-brand-neutral tracking-tight">
          {t.eventProductionPage.relatedLabel}
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {t.eventProductionPage.related.map((service) => (
            <a
              key={service.title}
              href={service.href}
